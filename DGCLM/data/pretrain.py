...
class AE(nn.Module):
    ...
    def forward(self, x):
        ...
        return x_bar, z.view(x.shape[0],-1)
...
def pretrain_ae(model, dataset):
    ...
    for epoch in range(30):
        ...
        with torch.no_grad():
            ...
            best_k, kmeans = k_model.best_k()
            print("best k:.........................:",best_k)
        torch.save(model.state_dict(), 'feature.pkl')
        print("saved finishing!")
model = AE(
        n_enc_1=32,
        n_enc_2=64,
        n_enc_3=128,
...
x = np.loadtxt('./dicom/output.txt', dtype=float)
print(x.shape)
