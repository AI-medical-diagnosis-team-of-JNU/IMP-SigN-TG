...
def construct_graph(features, best_k, method='heat'):
    fname = 'graph/feature'+str(best_k)+'_graph.txt'
    num = best_k
    dist = None
...
reut = np.loadtxt('data/dicom/output.txt', dtype=float)
#label = np.loadtxt('data/reut_label.txt', dtype=int)

construct_graph(reut, 6, 'ncos')
